...
export interface ComplianceRepository {
  ...
  listAdjustedCB(year: number): Promise<{ ship_id: string; cb_before: number }[]>;

  // Required for getAdjustedCB use-case
  getComplianceBalance(shipId: string, year: number): Promise<{ cb_gco2eq: number } | null>;

  getAppliedBankEntries(shipId: string, year: number): Promise<{ amount_gco2eq: number }[]>;
}
