...
export function makeComputeRouteMetrics(routeRepo: RouteRepository) {
  return async function computeRouteMetrics(routeId: string) {
    ...
    const energy_mj = Number(
      (route.fuel_tons * ENERGY_DENSITY[fuelType]).toFixed(2)
    );
    const emissions_gco2eq = Number(
      (route.fuel_tons * EMISSION_FACTORS[fuelType]).toFixed(2)
    );

    if (energy_mj === 0) throw new Error("Computed energy is zero");

    const intensity_gco2_per_mj = Number(
      (emissions_gco2eq / energy_mj).toFixed(4)
    );
